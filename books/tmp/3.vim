...
" ========================================================================
" ÒÀÔÆ(lilydjwg) µÄ vimrc
" ÎÒµÄ²©¿Í£º http://lilydjwg.is-programmer.com/
"
" Õû¸öÊÊÓÃÓÚ±¾ÈË *Only*
" ²»¹ı£¬ÆäÖĞµÄ²¿·ÖÅäÖÃºÜ¿ÉÄÜ·Ç³£ÊÊºÏÄãÅ¶¡«¡«
" ²»ÒªÕû¸öµØÕÕ°á£¬Ö»¡°³­Ï®¡±¶ÔÄã×Ô¼ºÓĞÓÃµÄ²¿·Ö£¡
"
" ÓĞÈÎºÎÒâ¼ûºÍ½¨Òé£¬»òÕßÆäËüÏëËµµÄ£¬¿ÉÒÔµ½ÎÒµÄ²©¿ÍÁôÑÔ¡£
"
" Ğí¿É£ºGPLv3
" ========================================================================
" ÆäËûÎÄ¼ş[[[1
...
"   ´ò¿ª NERDTree£¬Ê¹ÓÃµ±Ç°ÎÄ¼şÄ¿Â¼»òÕßµ±Ç°Ä¿Â¼[[[2
...
" Ê¹ÓÃ colorpicker ³ÌĞò»ñÈ¡ÑÕÉ«Öµ(hex/rgba)[[[2
...
" ¸ü¸Ä¹â±êÏÂµÄÑÕÉ«Öµ(hex/rgba/rgb)[[[2
...
function LookFurther(down)
	"À´Ô´ http://www.ibm.com/developerworks/cn/linux/l-vim-script-1/£¬ÓĞĞŞ¸Ä
	"Locate current column and preceding line from which to copy
	let column_num      = virtcol('.')
	let target_pattern  = '\%' . column_num . 'v.'
	let target_pattern_1  = '\%' . (column_num+1) . 'v.'

	" FIXed µ±¹â±êÎ»ÓÚÈçÏÂ | ËùÊ¾Î»ÖÃÊ±£¬½«È¡µÃ´íÎóµÄĞéÄâÁĞºÅ
	"          /ÖĞÎÄ
	"          |ÖĞÎÄ
	" ¹â±êÏÂµÄ×Ö·ûÊÇ¶à×Ö½ÚµÄ£¿
	" echo '['.matchstr(getline('.'), target_pattern).']'
	if matchstr(getline('.'), target_pattern) == '' &&
				\ matchstr(getline('.'), target_pattern_1) != ''
		let column_num -= 1
		" ÉÏÃæµÄ×Ö·û¿ÉÄÜÊÇÓ¢ÎÄ£¨Ç°Õß£©»òÕßÖĞÎÄ£¨ºóÕß£©µÄ
...
"   ·µ»Øµ±Ç°ÈÕÆÚµÄÖĞÎÄ±íÊ¾[[[2
function Lilydjwg_zh_date()
	let d = strftime("%YÄê%mÔÂ%dÈÕ")
	let d = substitute(d, '[ÄêÔÂ]\@<=0', '', 'g')
	return d
endfunction
"   ¹Ø±ÕÄ³¸ö´°¿Ú[[[2
...
function Lilydjwg_toggle_color()
	...
	" spring2 ÊÇÔö¼ÓÁË²ÊÉ«ÖÕ¶ËÖ§³ÖµÄ spring
...
function Lilydjwg_open_url()
	...
	if s:url == ""
		echohl WarningMsg
		echomsg 'ÔÚ¹â±ê´¦Î´·¢ÏÖURL£¡'
		echohl None
	else
		echo '´ò¿ªURL£º' . s:url
		if has("win32") || has("win64")
			" start ²»ÊÇ³ÌĞò£¬ËùÒÔÎŞĞ§¡£²¢ÇÒ£¬cmd Ö»ÄÜÊ¹ÓÃË«ÒıºÅ
...
syntax on
" set guifont=ÎÄÈªæäµÈ¿íÕıºÚ\ Medium\ 10
set number
set smarttab
set expandtab
" ²»ÒªÏìÁå£¬¸ü²»ÒªÉÁÆÁ
...
set history=10000
set wildmenu
set delcombine " ×éºÏ×Ö·ûÒ»¸ö¸öµØÉ¾³ı
set laststatus=2 " ×ÜÊÇÏÔÊ¾×´Ì¬À¸
" Ê×ÏÈ³¢ÊÔ×î³¤µÄ£¬½Ó×ÅÂÖ»»²¹È«Ïî
...
" Linux Óë Windows µÈ [[[2
if has("gui_macvim")
	set macmeta
end
if has("win32") || has("win64")
	" Win ±àÂë [[[3
	" ĞĞ½ûÔò²¹¶¡ÒªÇó encoding Îª 'utf-8'
	" µ«ÊÇÉèÖÃ encoding=utf-8 »áµ¼ÖÂ×´Ì¬À¸ºÍ±àÒëÕßĞÅÏ¢ÂÒÂë
	" set encoding=utf-8
...
if $LANGUAGE =~ '^zh' || ($LANGUAGE == '' && v:lang =~ '^zh')
	" »º³åÇøºÅ ÎÄ¼şÃû ĞĞÊı ĞŞ¸Ä °ïÖú Ö»¶Á ±àÂë »»ĞĞ·û BOM ======== ×Ö·û±àÂë Î»ÖÃ °Ù·Ö±ÈÎ»ÖÃ
	set statusline=%n\ %<%f\ %LĞĞ\ %{&modified?'[+]':&modifiable\|\|&ft=~'^\\vhelp\|qf$'?'':'[-]'}%h%r%{&fenc=='utf-8'\|\|&fenc==''?'':'['.&fenc.']'}%{&ff=='unix'?'':'['.&ff.']'}%{&bomb?'[BOM]':''}%{&eol?'':'[noeol]'}%=\ 0x%-4.8B\ \ \ \ %-14.(%l,%c%V%)\ %P
else
...
elseif has("unix")
	...
	" ·ÀÖ¹ÍË³öÊ±ÖÕ¶ËÂÒÂë
	" ÕâÀïÁ½Õß¶¼ĞèÒª¡£Ö»Ç°Õß±êÌâ»áÖØ¸´£¬Ö»ºóÕß»áÂÒÂë
	set t_fs=(B
	set t_IE=(B
	if &term =~ "256color"
		set cursorline
		...
		" ÔÚLinuxÎÄ±¾ÖÕ¶ËÏÂ·Ç²åÈëÄ£Ê½ÏÔÊ¾¿é×´¹â±ê
		...
		elseif $TERMCAP =~ 'Co#256'
			...
			" ÔİÊ±Ö»ÓĞÕâ¸öÅäÉ«±È½ÏÊÊºÏÁË
			colorscheme default
			" ÔÚÖÕ¶ËÏÂ£¬Èç¹ûÂë±í´æÔÚ£¬Ôò×Ô¶¯¼ÓÔØvimimÊäÈë·¨
			if len(split(globpath(&rtp, 'so/vimim.wubi.txt'), '\n')) > 0
				runtime so/vimim.vim
			endif
		endif
	endif
	" ÔÚ²»Í¬Ä£Ê½ÏÂÊ¹ÓÃ²»Í¬ÑÕÉ«µÄ¹â±ê
	" ²»ÒªÔÚ ssh ÏÂÊ¹ÓÃ
...
"   cmap [[[2
"     »¹ÊÇÕâÑù°É
"     FIXME µ«ÕâÑùÔÚ wildmenu ²¹È«Ê±»áÓĞµãÆæ¹Ö
cmap <C-P> <Up>
cmap <C-N> <Down>
...
" ×Ô¶¯ÃüÁî[[[1
"   ×Ô¶¯¹Ø±ÕÔ¤ÀÀ´°¿Ú£¨²»ÄÜÓÃÔÚÃüÁî´°¿Ú£¬ËùÒÔÉèÖÃÁËÒ»¸ö±äÁ¿£©
let s:cmdwin = 0
...
" É¾³ıµ±Ç°ÎÄ¼ş
command Delete if delete(expand('%')) | echohl WarningMsg | echo "É¾³ıµ±Ç°ÎÄ¼şÊ§°Ü" | echohl None | endif
...
"   ÓÃ VimExplorer ²å¼ş´ò¿ªµ±Ç°ÎÄ¼şËùÔÚµÄÄ¿Â¼
command Path VE %:p:h
...
"   ConflictMotions [[[2
" ½ûÓÃ \x ¿ªÍ·µÄÓ³Éä£»ËüÃÇÓë EnhancedCommentify ³åÍ»ÁË
...
"  À´Ô´ http://gist.github.com/476387
...
"   2html.vim, Ê¹ÓÃXHTML¸ñÊ½[[[2
let use_xhtml = 1
"   shell ½Å±¾´ò¿ªÕÛµş
let g:sh_fold_enabled = 3 " ´ò¿ªº¯ÊıºÍ here ÎÄµµµÄÕÛµş
"   Align[[[2
...
"   syntax/vim.vim Ä¬ÈÏ»á¸ßÁÁ s:[a-z] ÕâÑùµÄº¯ÊıÃûÎª´íÎó[[[2
...
"   xml.vim£¬Ê¹ËùÓĞµÄ±êÇ©¶¼¹Ø±Õ[[[2
let xml_use_xhtml = 1
"   netrw£¬elinks²»ĞĞ£¬Ê¹ÓÃcurl°É
if executable("curl")
...
if has("cscope") && executable("cscope")
	...
	" Ó³Éä [[[2
	" ²éÕÒCÓïÑÔ·ûºÅ£¬¼´²éÕÒº¯ÊıÃû¡¢ºê¡¢Ã¶¾ÙÖµµÈ³öÏÖµÄµØ·½
	nmap css :cs find s <C-R>=expand("<cword>")<CR><CR>
	" ²éÕÒº¯Êı¡¢ºê¡¢Ã¶¾ÙµÈ¶¨ÒåµÄÎ»ÖÃ£¬ÀàËÆctagsËùÌá¹©µÄ¹¦ÄÜ
	...
	" ²éÕÒegrepÄ£Ê½£¬Ïàµ±ÓÚegrep¹¦ÄÜ£¬µ«²éÕÒËÙ¶È¿ì¶àÁË
	nmap cse :cs find e <C-R>=expand("<cword>")<CR><CR>
	" ²éÕÒ²¢´ò¿ªÎÄ¼ş£¬ÀàËÆvimµÄfind¹¦ÄÜ
